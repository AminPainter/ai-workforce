...
export class SnackTrackerListener {
  ...
  @OnEvent(SLACK_BAKAR_MENTION_EVENT)
  async onBakarMention({ thread, message }: SlackBakarEvent): Promise<void> {
    try {
      await this.snackTrackerService.handleCommand(thread, message);
    } catch (error) {
      this.logger.error(`bakar command handling failed: ${error}`);
    }
  }
}
