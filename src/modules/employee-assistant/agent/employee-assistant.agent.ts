...
export function createEmployeeAssistant(
  ...
  return new ToolLoopAgent({
    ...
    // real channel. The empty default means "no channel", so a call that skips the
    // override refuses to settle — settlement is only allowed from #bakar.
    toolsContext: { markSnacksFulfilled: {} },
  });
}
